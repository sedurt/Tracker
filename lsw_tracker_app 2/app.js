...
function renderToday() {
  ...
  ["win", "improve", "notes"].forEach(name => {
    ...
    el.value = entry[name] || "";
    el.oninput = () => { entry[name] = el.value; saveState(); renderHistory(); };
  });
}
...
function renderGoalList(containerId, goals, onDelete) {
  ...
  goals.forEach(goal => {
    ...
    div.querySelector("button").onclick = () => onDelete(goal.id);
...
function renderSettings() {
  ...
  state.habits.forEach(habit => {
    ...
    input.oninput = e => { habit.name = e.target.value; saveState(); renderToday(); renderTrends(); renderHistory(); };
...
document.getElementById("addHabit").onclick = () => {
  ...
  if (name) { state.habits.push({ id: makeId(), name, active: true }); saveState(); render(); }
};
document.getElementById("addGoal").onclick = () => {
  const name = prompt("Overall goal name");
  if (name) { state.goals.push({ id: makeId(), name, target: 1, current: 0 }); saveState(); renderGoals(); }
};
...
document.getElementById("importBackup").onchange = e => {
  ...
  reader.onload = () => {
    try { state = normalizeState(JSON.parse(reader.result)); saveState(); render(); alert("Backup imported."); }
