...
function renderGoalList(containerId, goals, onDelete, goalTypeLabel = "goal") {
  ...
  goals.forEach(goal => {
    ...
    div.innerHTML = `<div class="goal-top"><label class="goal-name-label"><span class="field-label">Goal name</span><input class="goal-name-input" type="text"></label><button class="small-btn danger">Delete</button></div>
      <div class="setting-grid"><label><span class="field-label">Current</span><input type="number"></label><label><span class="field-label">Target</span><input type="number"></label><span>${pct}%</span></div>
      <div class="goal-progress"><div style="width:${pct}%"></div></div>`;
    const inputs = div.querySelectorAll("input");
    inputs[0].value = goal.name || "";
    inputs[1].value = goal.current;
    inputs[2].value = goal.target;
    inputs[0].oninput = e => { goal.name = e.target.value; saveState(); };
    inputs[1].onchange = e => { goal.current = Number(e.target.value); saveState(true); renderGoals(); };
    inputs[2].onchange = e => { goal.target = Number(e.target.value || 1); saveState(true); renderGoals(); };
...
function focusInputByValue(containerId, value) {
  requestAnimationFrame(() => {
    const container = document.getElementById(containerId);
    if (!container) return;
    const input = [...container.querySelectorAll("input, textarea")].find(el => el.value === value) || container.querySelector("input, textarea");
    if (input) {
      input.focus();
...
document.getElementById("addHabit").onclick = () => {
  const habit = { id: makeId(), name: "New habit", active: true };
  state.habits.push(habit);
  saveState(true);
  renderCurrentScreen();
  focusInputByValue("habitSettings", "New habit");
};
document.getElementById("addGoal").onclick = () => {
  state.goals.push({ id: makeId(), name: "New overall goal", target: 1, current: 0 });
  saveState(true);
  renderGoals();
  focusInputByValue("goalList", "New overall goal");
};
document.getElementById("addMonthlyGoal").onclick = () => {
  getMonthlyGoals().push({ id: makeId(), name: "New monthly goal", target: 1, current: 0 });
  saveState(true);
  renderGoals();
  focusInputByValue("monthlyGoalList", "New monthly goal");
};
