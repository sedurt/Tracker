...
const defaultState = {
  settings: { appName: "LSW Tracker", accent: "#2563eb", dark: false, cloud: { sheetUrl: "", secret: "", autoSync: false, deviceId: makeId(), lastSync: "", lastSyncStatus: "" } },
  habits: [
...
function normalizeState(input) {
  ...
  s.settings.cloud = { ...defaultState.settings.cloud, ...((input.settings || {}).cloud || {}) };
  if (!s.settings.cloud.deviceId) s.settings.cloud.deviceId = makeId();
...
function saveState(immediate = false, skipCloud = false) {
  const write = () => localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  if (immediate) {
    if (saveTimer) clearTimeout(saveTimer);
    saveTimer = null;
    write();
    if (!skipCloud) queueCloudSync("auto");
    return;
  }
  if (saveTimer) clearTimeout(saveTimer);
...
function flushSave() { saveState(true, true); }
function cloudReady() {
  const c = state.settings.cloud || {};
  return !!(c.sheetUrl && c.secret);
}
function cloudStatusText() {
  const c = state.settings.cloud || {};
  if (!c.sheetUrl) return "Not connected.";
  const auto = c.autoSync ? "Auto-backup on" : "Auto-backup off";
  const last = c.lastSync ? ` Last sync sent: ${new Date(c.lastSync).toLocaleString()}.` : " No sync sent yet.";
  return `${auto}.${last}${c.lastSyncStatus ? " " + c.lastSyncStatus : ""}`;
}
function updateCloudStatus() {
  const el = document.getElementById("cloudStatus");
  if (el) el.textContent = cloudStatusText();
}
function queueCloudSync(syncType = "auto") {
  const c = state.settings.cloud || {};
  if (!c.autoSync || !cloudReady()) return;
  if (cloudSyncTimer) clearTimeout(cloudSyncTimer);
  cloudSyncTimer = setTimeout(() => { syncToGoogleSheets(syncType); }, 2200);
}
function buildCloudPayload(syncType = "manual") {
  return {
    secret: state.settings.cloud.secret,
    syncType,
    appVersion: "2.5",
    deviceId: state.settings.cloud.deviceId,
    syncedAt: new Date().toISOString(),
    state
  };
}
async function syncToGoogleSheets(syncType = "manual") {
  const c = state.settings.cloud || {};
  if (!cloudReady()) {
    alert("Paste your Google Apps Script URL and secret token first.");
    return;
  }
  try {
    await fetch(c.sheetUrl, {
      method: "POST",
      mode: "no-cors",
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify(buildCloudPayload(syncType))
    });
    state.settings.cloud.lastSync = new Date().toISOString();
    state.settings.cloud.lastSyncStatus = "Sent to Google Sheets. Check the Sheet to confirm it saved.";
    saveState(true, true);
    updateCloudStatus();
    if (syncType === "manual") alert("Sync sent. Open your Google Sheet and check the tabs to confirm it saved.");
  } catch (err) {
    state.settings.cloud.lastSyncStatus = "Sync failed. Check the URL and your connection.";
    saveState(true, true);
    updateCloudStatus();
    if (syncType === "manual") alert("Sync failed. Check the URL and your connection.");
  }
}
...
function renderSettings() {
  ...
  const cloud = state.settings.cloud || (state.settings.cloud = structuredClone(defaultState.settings.cloud));
  document.getElementById("sheetUrlInput").value = cloud.sheetUrl || "";
  document.getElementById("sheetSecretInput").value = cloud.secret || "";
  document.getElementById("autoSyncInput").checked = !!cloud.autoSync;
  updateCloudStatus();
...
document.getElementById("sheetUrlInput").onchange = e => { state.settings.cloud.sheetUrl = e.target.value.trim(); saveState(true, true); updateCloudStatus(); };
document.getElementById("sheetSecretInput").onchange = e => { state.settings.cloud.secret = e.target.value.trim(); saveState(true, true); updateCloudStatus(); };
document.getElementById("autoSyncInput").onchange = e => { state.settings.cloud.autoSync = e.target.checked; saveState(true, true); updateCloudStatus(); };
document.getElementById("syncNow").onclick = () => syncToGoogleSheets("manual");
document.getElementById("clearCloudSettings").onclick = () => {
  if (!confirm("Clear the Google Sheets backup URL and secret from this phone? This will not delete anything already saved in Google Sheets.")) return;
  state.settings.cloud.sheetUrl = "";
  state.settings.cloud.secret = "";
  state.settings.cloud.autoSync = false;
  state.settings.cloud.lastSync = "";
  state.settings.cloud.lastSyncStatus = "";
  saveState(true, true);
  renderSettings();
};
